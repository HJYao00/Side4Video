...
class DividedST(nn.Module):
    ...
    def forward(self, x):
        ...
        nt, b, c = xt.size()
        n = nt // self.T
        xt = rearrange(xt, '(n t) b c -> t (b n) c', n=n, t=self.T, b=b, c=c)
        # [t bn c] for temporal attn
        res_temporal = self.drop_path(self.attention(self.temporal_ln(xt)))  # t bn c
        res_temporal = rearrange(res_temporal, 't (b n) c -> (n t) b c', n=n, t=self.T, b=b, c=c)
        xt = x[1:, :, :] + res_temporal  # nt b c

        # prepare [1+n bt c] for spatial attn
        init_cls_token = x[0].unsqueeze(0) # 1 b c
        cls_token = init_cls_token.repeat(self.T, 1, 1)  # t b c
        cls_token = cls_token.view(-1, c).unsqueeze(0)  # 1 bt c
        xs = xt
        xs = rearrange(xs, '(n t) b c -> n (b t) c', n=n, t=self.T, b=b, c=c)
        xs = torch.cat((cls_token, xs), 0)  # 1+n bt c
        
        x_mlp = torch.cat((init_cls_token, xt), 0)  # 1+nt b c
        return xs, x_mlp
    ...
    #     n, bt, c = xt.size()
    #     b = bt // self.n_segment
    #     xt = xt.view(n, b, self.n_segment, c).permute(2, 1, 0, 3).contiguous()  # t b n c
    #     xt = xt.view(self.n_segment, b * n, c) # t bn c
    #     res_temporal = self.drop_path(self.attention(self.temporal_ln(xt)))
    #     res_temporal = res_temporal.view(self.n_segment, b, n, c).permute(2, 1, 0, 3).contiguous().view(n, bt, c)
...
class PostMLP(nn.Module):
    ...
    def forward(self, x, x_mlp):
        ...
        bt, c = cls_token.size()
        b = bt // self.T
        cls_token = cls_token.view(b, self.T, c)
        cls_token = torch.mean(cls_token, 1).unsqueeze(0)
        res_spatial = x[1:,:,:] # n bt c
        n = res_spatial.size(0)
        res_spatial = rearrange(res_spatial, 'n (b t) c -> (n t) b c', n=n, b=b, t=self.T, c=c)
        x = x_mlp + torch.cat((cls_token, res_spatial), 0) # 1+nt b c
        return x
