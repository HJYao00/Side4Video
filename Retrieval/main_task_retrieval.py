...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument("--freeze_vit_encoder", action='store_true', help='whether freeze vit encoder')
    parser.add_argument('--dist_url', default='env://',
                        help='url used to set up distributed training')
    parser.add_argument('--interaction', default='None')
    parser.add_argument('--cdcr', action='store_true')
    parser.add_argument("--amp", action='store_true', help='whether amp training')
...
def set_seed_logger(args):
    ...
    if args.local_rank == 0:
        shutil.copy('modules/module_clip.py', args.output_dir)
...
def init_model(args, device, n_gpu, local_rank):
    if args.init_model and 'EVA' not in args.pretrained_clip_name:
    ...
    if 'EVA' in args.pretrained_clip_name:
        from eva_clip.eva_modeling import CLIP4CLIP
        model = CLIP4CLIP(args)
    elif 'ViT' in args.pretrained_clip_name:
    ...
    if args.init_model:
        ...
        #     if 'side' in k:
        #         new_name = 'clip.' + k
        #         checkpoint['model_state_dict'][new_name] = checkpoint['model_state_dict'][k]
...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    decay_clip_param_tp = [(n, p) for n, p in decay_param_tp if "clip." in n and 'side' not in n]
    decay_noclip_param_tp = [(n, p) for n, p in decay_param_tp if "clip." not in n or 'side' in n]

    no_decay_clip_param_tp = [(n, p) for n, p in no_decay_param_tp if "clip." in n and 'side' not in n]
    no_decay_noclip_param_tp = [(n, p) for n, p in no_decay_param_tp if "clip." not in n or 'side' in n]
    ...
    scheduler = None # _lr_scheduler(args, optimizer)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if scheduler is not None:
                scheduler.step(epoch + step / len(train_dataloader))  # Update learning rate schedule
...
def main():
    global logger
    args = get_args()
    init_distributed_mode(args) # 初试化
    args = set_seed_logger(args)    # 随机种子和日志
    ...
    if hasattr(model, "clip") and args.freeze_layer_num > -1:
        for name, param in model.clip.named_parameters():
            ...
            if name.find("ln_final.") == 0 or name.find("text_projection") == 0 or name.find("logit_scale") == 0 \
                    or name.find("visual.ln_post.") == 0 or name.find("visual.proj") == 0 or name.find('visual.head')==0 or name.find('visual.norm')==0 or name.find('text.ln_final')==0:
            ...
            # if 'side' in name or args.linear_patch == "3d" and name.find("conv2."):
            #     param.requires_grad = True
            #     continue
            # else:
    ...
    if DATALOADER_DICT[args.datatype]["val"] is not None:
        val_dataloader, val_length = DATALOADER_DICT[args.datatype]["val"](args, tokenizer, subset="val")  # through this
    else:
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            try:
                if args.local_rank == 0:
                    ...
                    # if args.datatype == "msvd" or args.datatype == 'vatex':
                    #     logger.info('Eval later')
                    #     continue
