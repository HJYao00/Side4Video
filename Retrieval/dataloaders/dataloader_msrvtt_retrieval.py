    ...
    def _get_rawframes(self, choice_video_ids):
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            if len(raw_video_data.shape) > 3:
                ...
                try:
                    assert video_slice.shape == (self.max_frames, 1, 3, self.rawFramesExtractor.size, self.rawFramesExtractor.size)
                except:
...
class MSRVTT_TrainDataLoader(Dataset):
    ...
    def _get_rawframes(self, choice_video_ids):
        ...
        # Pair x L x T x 3 x H x W
        video = np.zeros((len(choice_video_ids), self.max_frames, 1, 3,
                          self.rawFramesExtractor.size, self.rawFramesExtractor.size), dtype=np.float)  # (1, 8, 1, 3, 224, 224)
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            if len(raw_video_data.shape) > 3:
                ...
                video_slice = self.rawFramesExtractor.process_frame_order(video_slice, frame_order=self.frame_order)    # (8, 1, 3, 224, 224)
