...
def conv_5x3x3(inp, oup, groups=1):
    return nn.Conv3d(inp, oup, (5, 3, 3), (1, 1, 1), (2, 1, 1), groups=groups)

def conv_5x1x1(inp, oup, groups=1):
    return nn.Conv3d(inp, oup, (5, 1, 1), (1, 1, 1), (2, 0, 0), groups=groups)

def conv_3x1x1(inp, oup, groups=1):
    return nn.Conv3d(inp, oup, (3, 1, 1), (1, 1, 1), (1, 0, 0), groups=groups)

def conv_7x5x5(inp, oup, groups=1):
    return nn.Conv3d(inp, oup, (7, 5, 5), (1, 1, 1), (3, 2, 2), groups=groups)

def conv_5x5x5(inp, oup, groups=1):
    return nn.Conv3d(inp, oup, (5, 5, 5), (1, 1, 1), (2, 2, 2), groups=groups)

def conv_7x7x7(inp, oup, groups=1):
    return nn.Conv3d(inp, oup, (7, 7, 7), (1, 1, 1), (3, 3, 3), groups=groups)

def generate_drop_layer(start:int, end:int, drop_rate=None, drop_layer_mode=None):
    drop_layer = []
    if drop_layer_mode == 'random':
        for i in range(start, end):
            random_num = np.random.uniform()
            if random_num < drop_rate:
                drop_layer.append(i)
    elif drop_layer_mode == 'fix random':
        drop_layer = [i for i in range(start)]
        layers = [i for i in range(start, end)]
        random_drop_layer = sample(layers, drop_rate)
        drop_layer.extend(random_drop_layer)
    return drop_layer 
...
class AttnCBlock(nn.Module):
    def __init__(self, dim, side_dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.attn = nn.MultiheadAttention(dim, dim//64, dropout=0., batch_first=True)
...
class EVAVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 pt_hw_seq_len=16, intp_freq=False, naiveswiglu=False, subln=False,T=12,side_dim=320,):
        super().__init__()
        ...
        self.side_lns = []
        self.side_blocks = []
        self.drop_layer_mode = 'fix random'
        self.side_start_layer = 0
        self.drop_layer = [i for i in range(0, depth, 2)]
        if self.drop_layer_mode == 'fix random':
            self.drop_layer_param = 0
        elif self.drop_layer_mode == 'random':
            self.drop_layer_param = 0.2
        for i in range(depth):
        ...
        self.side_proj = nn.Parameter(torch.zeros(self.side_dim, embed_dim))
